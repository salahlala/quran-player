...
let hidBackgroundButton = document.querySelector(".main-hide .hid-background");
let backgroundChanger = document.querySelector(".background-changer");
let hidValue = false;

hidBackgroundButton.addEventListener("click", () => {
  if (hidBackgroundButton.classList.contains("active")) {
    regex = randomBack.src
      .toString()
      .replace(/http:\/\/127.0.0.1:5500/gi, "..");
    randomBackground.style.backgroundImage = `url(${regex})`;
    backgroundChanger.classList.remove("hid");
  } else {
    randomBackground.style.backgroundImage = "";
    backgroundChanger.classList.add("hid");
  }
  hidBackgroundButton.classList.toggle("active");
});
...
// remove active class on click container
mainContainer.addEventListener("click", () => {
  volumeBtn.classList.remove("active");
  hadithBtn.classList.remove("active");
  azkarBtn.classList.remove("active");
});
// remove active class on click the background effect
  ...
  hadith.addEventListener("click", (e) => {
    ...
    fetch(`https://alquranbd.com/api/hadith/${shikName}/3`)
      ...
      .then((hadText) => {
        ...
        hTitle.className = "hadith-title flex";
