...
let changeAudio = document.querySelector(".change-audio");
...
ayaName.onclick = function () {
  changeAudio.classList.add("active");
  // backgroundEffect.classList.add("active");
  showEffect();
};
...
document.addEventListener("click", (e) => {
  if (e.target.className == "container") {
    changeAudio.classList.remove("active");
    ...
    .then((text) => {
      for (let sura in text.data) {
        changeAudio.innerHTML += `
          <div id="${text.data[sura].number}">
            <p><span>(${text.data[sura].number})</span>${text.data[sura].asma.ar.long}</p>
          </di>
        `;
      }

      let allSurah = changeAudio.querySelectorAll("div");

      setTimeout(() => {
        allSurah[Math.floor(Math.random() * allSurah.length)].click();
        selectInd = Math.floor(Math.random() * allSurah.length);
        // window.changeAll(3);
      }, 1000);

      allSurah.forEach((div, indx) => {
        div.addEventListener("click", () => {
          loadingAnimation.classList.remove("active");
          fetch(
            `https://api.alquran.cloud/v1/surah/${indx + 1}/ar.${shikhName}`
          )
            ...
            .then((data) => {
              ...
              nameOfAya = data.data.name;
              getIndx = indx;
              changeAudio.scrollTo({
                top: div.offsetTop - 15,
                behavior: "smooth",
              });
              testIndex = 0;
              allSurah.forEach((div) => div.classList.remove("active"));
      ...
      let suras = changeAudio.querySelectorAll("div");
      makeAudio.addEventListener("ended", () => {
        testIndex++;
        if (testIndex == audio.length) {
          if (getIndx + 1 == 114) {
            suras[0].click();
          } else {
            suras[getIndx + 1].click();
          }
        }
      });
      ...
      mainSelect.addEventListener("input", () => {
        ...
        fetch(
          `https://api.alquran.cloud/v1/surah/${getIndx + 1}/ar.${shikhName}`
        )
          .then((response) => response.json())
          .then((data) => {
            aya = [];
...
if (window.screen.width <= 767) {
  mainSelect.onfocus = function () {
    mainSelect.size = 5;
  };

  mainSelect.onblur = function () {
    mainSelect.size = 1;
  };
